#Notes: If you would like to run the application from R after downloading the working directory from Github, you need to run through 
#all the steps on this file. These steps import data files that are included in the Github directory - so you need to 
#make sure they are downloaded and present in the R working directory. These files constitute the whole compendium of RNA-seq data sets.
...
#3. Load in all count tables, design matrices, and metadata for each species [Code will take less than 30 seconds to run]
{
  ...
  #Pseudomonas Aeruginosa
  {
    ...
    #Special code
    names(CFSeq_Data[[9]])[5] <- "GSE112911a"
    names(CFSeq_Data[[9]])[6] <- "GSE112911b"
...
{
  Files <- str_sort(list.files(path = "./Pathway_Data/Full_Annotations", pattern = "*.csv"))
  GenePathway_Data <- vector(mode = "list", length(Files))
  names(GenePathway_Data) <- Files
  
  for (i in 1:length(GenePathway_Data)) {
    GenePathway_Data[[i]] <- read.csv(paste0("./Pathway_Data/Full_Annotations/", Files[i]))
  }
  ...
  Species_Numbers <- c(1,1,3,3,3,3,2,2,4,4,5,5,7,7,7,7,8,9,9,9,9,9,10,10,11,11,11,11,11,12,12,13,13,13,13)
  
}

#6. Create R Data Object 
{
  save(CFSeq_Data, GenePathway_Data, Species_Numbers, file = "Data.Rdata")
}

#--- You don't need to run the following lines, they are just for viewing information about the app
{
  ...
  #Number of studies currently in the application
  NumStudies <- 0
  for (i in 1:length(CFSeq_Data)) {
    NumStudies = NumStudies + length(CFSeq_Data[[i]])
    print(NumStudies) #Shows number of studies per species
  }
  print(NumStudies) #Total number of studies = 147
  
  #Number fo samples currently in the application
  NumSamples <- 0
  for (i in 1:length(CFSeq_Data)) {
    for (j in 1:length(CFSeq_Data[[i]])) {
      NumSamples <- NumSamples + (dim(CFSeq_Data[[i]][[j]]$Count_Table)[2] - 1)
    }
    print(NumSamples) #Shows number of samples per species
  }
  print(NumSamples) #Total number of samples = 1446
