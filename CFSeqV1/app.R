  ...
  #4. Press Show Analysis Button --> Show Analysis Screen with default comparison 
  {
      shiny::observeEvent(input$ShowAnalysis, {
        ...
        #Log App Usage Data
        {
          ...
          Session.ID <- gsub(":", "", Session.ID, fixed = TRUE)
          Session.ID <- paste(names(CFSeq_Data)[SpeciesID], Session.ID, sep = "_")
          Session.ID <- paste(FilterInput[[as.integer(substr(input$current_id,11,15))]][[3]]$Species, Session.ID, sep = "_")
          Session.ID <- paste(FilterInput[[as.integer(substr(input$current_id,11,15))]][[3]]$Strain, Session.ID, sep = "_")
          Session.ID <- paste(FilterInput[[as.integer(substr(input$current_id,11,15))]][[3]]$Treatment, Session.ID, sep = "_")
          Session.ID <- paste(FilterInput[[as.integer(substr(input$current_id,11,15))]][[3]]$Medium, Session.ID, sep = "_")
          Session.ID <- paste(FilterInput[[as.integer(substr(input$current_id,11,15))]][[3]]$Genotype, Session.ID, sep = "_")
          Session.ID <- paste(FilterInput[[as.integer(substr(input$current_id,11,15))]][[3]]$GEO.Accession, Session.ID, sample(1000:9999, 1), sep = "_")
        ...
        #Update Comparison list and pathways for specific study 
        {
          ...
          for (j in 1:length(TestGenes)) {
            for (i in 1:length(GenePathway_Data_Filtered)) {
              if (length(grep(TestGenes[j], GenePathway_Data_Filtered[[i]]$Gene)) != 0) {
                PathwaySelect <- unique(GenePathway_Data_Filtered[[i]]$Pathway)
                PathwayFile <- GenePathway_Data_Filtered[[i]]
              }
            }
          }
                ...
                #Highlight a pathway on the Volcano or MA Plot
                {
                  observeEvent(input$Find_Pathway, {
                    ...
                    if (input$Find_Pathway != "" & input$Find_Pathway != "[Select a Pathway]") {
                      ...
                      Selected_Pathway <- PathwayFile2[which(PathwayFile2$Pathway == input$Find_Pathway),]
