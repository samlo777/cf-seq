...
server <- function(input, output, session) {
  ...
  observeEvent(input$Select_Species, {
    ...
    {
      VALUES <- reactiveValues(CFSeqStrain = NULL, CFSeqTreatment  = NULL, CFSeqMedium = NULL, CFSeqGenotype = NULL, CFSeqSubSpecies = NULL, CFSeq_Data = CFSeq_Data, CFSeq_Genes = CFSeq_Genes, S = NULL) 
      output$table <- NULL
      Strains <- vector()
      Treatments <- vector()
      Media <- vector()
    ...
    #Action Button: Confirm filter selections
    {
      observeEvent(input$button, {
        ...
        #Action Button: More Details Dialogue + Run Analysis Window (1)
        observeEvent(input$current_id, {
          ...
          #Show Dialog Window with study info + Run Analysis Button 
          {
            ...
            if (input$Select_Strain == "[Select A Strain]" & input$Select_Treatment == "[Select A Treatment]" & input$Select_Medium == "[Select A Medium]" & input$Select_Genotype == "[Select A Gene Perturbation]") {
              ...
              for (i in 1:length(CFSeq_Data[[VALUES$S]])) {
                if (CFSeq_Data[[VALUES$S]][[i]][[3]]$GEO.Accession %in% DF1$GEO_Accession) {
          ...
          #Provide different output for studies with viable comparisons and no viable comparisons
      ...
      #Action Button: More Details Dialogue + Run Analysis Window (2)
      observeEvent(input$current_id, {
